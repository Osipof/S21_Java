...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) {
        ...
        String lDateTimeQueryPart = "ldatetime = '" + localDateTime + "' ";
        if (localDateTime == null) {
            lDateTimeQueryPart = lDateTimeQueryPart.replace("'", "");
        }
        String newText = message.getText();
        String textQueryPart = "text = '" + newText + "', ";
        if (newText == null) {
            textQueryPart = "text = '', ";
        }

        String updateMessageQuery = "UPDATE chat.message SET " +
                "sender_id = " + message.getAuthor().getId() + ", " +
                "room_id = " + message.getRoom().getId() + ", " +
                textQueryPart +
                lDateTimeQueryPart +
                "WHERE id = " + message.getId() + ";";

        System.out.println(updateMessageQuery);
